...
public class ReservationEventListener {
    ...
    public void handleReservationCreated(ReservationCreatedEvent event) {
        try {
            ...
            long activeReservationsCount = getActiveReservationsCount(memberId);
            logger.info("Member {} has {} active reservations, max allowed: {}", 
                    memberId, activeReservationsCount, member.getMaxConcurrentBookings());
            
            if (activeReservationsCount >= member.getMaxConcurrentBookings() && !member.isSuspended()) {
            ...
            if ("CANCELLED".equals(event.getNewStatus()) || "COMPLETED".equals(event.getNewStatus())) {
                ...
                if (member.isSuspended()) {
                    ...
                    logger.info("Member {} now has {} active reservations after status change", 
                            memberId, activeReservationsCount);
                    
                    if (activeReservationsCount < member.getMaxConcurrentBookings()) {
    ...
    private long getActiveReservationsCount(Long memberId) {
        try {
            String url = "http://localhost:8083/api/reservations/member/" + memberId + "/active/count";
            Long count = restTemplate.getForObject(url, Long.class);
            return count != null ? count : 0;
        } catch (Exception e) {
            logger.error("Error counting active reservations for member {}", memberId, e);
            return 0;
        }
    }
}
